...
### 2. Filter out sequences smaller than 20 bp
...
## Step X: Perform Local Alignments (BLAST)
...
## Step X: Perform Global Alignment (Needle)
...
## Step X: Perform Assembly to Assembly Alignment

### 1. Alignment

- Inputs
  - new assembly without chloroplast and mitochondria in `.fasta` format
    - `assemblies/dans_new_nuclear_assembly.fasta`
...
```bash
minimap2 -x asm5 assemblies/dans_new_nuclear_assembly.fasta assemblies/old_nuclear_assembly.fasta > minimap_outputs/oldToNewMap.paf
```
...
```bash
source ./utils/chr_sizes/get_sizes.sh assemblies/dans_new_nuclear_assembly.fasta > ./chr_sizes/new_nuclear_assembly_chr_sizes.txt
```
...
## Step X: Figure out how many remaps you'd expect from the old assembly to new assembly remapping
...
## Step X: Update the BLAST file with the all the necessary info for filtering
...
## Step X: Filter the BLAST file to perform reciprical best hit and figuring out which BLAST hits are accurate remaps of old features
...
## Step X: Update and clean up the remapped GFF file
